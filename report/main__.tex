...
%\includegraphics[width=0.3\textwidth]{sbu_logo.jpg}\\[1.0cm]
...
Figure~\ref{fig:logreturns} shows the full daily log return series 
for all five assets over the 20-year sample period. Volatility 
clustering is clearly visible across all assets, most prominently 
during the 2008--2009 financial crisis, the March 2020 COVID shock, 
and the 2022 rate hike cycle. This clustering is the primary 
motivation for GARCH filtering. Individual asset plots are provided 
in Appendix~\ref{app:returns}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig7_log_returns.png}
  \caption{Daily log return series for all five asset classes 
  (2005--2025) arranged in a 2-column grid. Volatility clustering 
  is evident across all assets.}
...
Figure~\ref{fig:qq} presents NIG quantile-quantile plots for all 
five assets. Full three-model comparisons (Gaussian, Student T, NIG) 
are provided in Appendix~\ref{app:qq}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig8_qq_nig.png}
  \caption{NIG QQ plots for all five asset classes. Points on the 
  45-degree reference line indicate perfect fit. NIG achieves close 
  alignment across all assets, particularly in the tails where VaR 
  accuracy is most critical.}
  \label{fig:qq}
\end{figure}

The tight clustering of points around the 45-degree line across 
all five assets confirms NIG's suitability as the innovation 
distribution. Systematic S-shaped deviations visible in the 
Gaussian QQ plots (Appendix~\ref{app:qq}) are absent here, 
confirming that NIG successfully captures the heavy-tail behaviour 
present in the standardised innovations.
...
\newpage
\section{Individual Log Return Series}
\label{app:returns}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig7_returns_GSPC.png}
  \caption{Log returns --- S\&P 500 (\texttt{\^{}GSPC})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig7_returns_AAPL.png}
  \caption{Log returns --- Apple Inc.\ (\texttt{AAPL})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig7_returns_GLD.png}
  \caption{Log returns --- SPDR Gold ETF (\texttt{GLD})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig7_returns_TIP.png}
  \caption{Log returns --- iShares TIPS ETF (\texttt{TIP})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig7_returns_EURUSD_X.png}
  \caption{Log returns --- EUR/USD (\texttt{EURUSD=X})}
\end{figure}

\newpage
\section{QQ Plots --- Three-Model Comparison}
\label{app:qq}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig8_qq_GSPC.png}
  \caption{QQ plots --- S\&P 500 (\texttt{\^{}GSPC})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig8_qq_AAPL.png}
  \caption{QQ plots --- Apple Inc.\ (\texttt{AAPL})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig8_qq_GLD.png}
  \caption{QQ plots --- SPDR Gold ETF (\texttt{GLD})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig8_qq_TIP.png}
  \caption{QQ plots --- iShares TIPS ETF (\texttt{TIP})}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/fig8_qq_EURUSD_X.png}
  \caption{QQ plots --- EUR/USD (\texttt{EURUSD=X})}
\end{figure}
